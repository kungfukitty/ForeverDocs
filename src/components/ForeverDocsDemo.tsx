...
const rand = (n = 8) => [...crypto.getRandomValues(new Uint8Array(n))].map(b => b.toString(16).padStart(2, '0')).join('')
const base36 = (len = 6) => [...Array(len)].map(() => (Math.floor(Math.random() * 36)).toString(36)).join('').toUpperCase()

const fakeDigest = () => rand(32)
const makeCode = (family = 'FD') => `${family.slice(0,2).toUpperCase()}-${new Date().getFullYear()}-${base36(3)}`
...
export default function ForeverDocsDemo() {
  ...
  const [step, setStep] = useState<'home'|'upload'|'review'|'crest'|'verify'>('home')
  ...
  const Outline = ({children}:{children: React.ReactNode}) => (
    <div className="rounded-2xl border border-zinc-800/60 bg-zinc-900/40 shadow-xl shadow-black/30 p-6">{children}</div>
  )
  const Pill = ({label}:{label:string}) => (
    <span className="text-xs rounded-full border border-zinc-700 px-2 py-1 uppercase tracking-wide text-zinc-300">{label}</span>
  )
  ...
  const Input = (props:any) => (
    <input {...props} className={`w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2 outline-none focus:border-zinc-500 ${props.className||''}`} />
  )
  const Select = (props:any) => (
    <select {...props} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2 outline-none focus:border-zinc-500">
      {props.children}
    </select>
  )
  const Button = ({children, onClick, variant = 'primary'}:{children:React.ReactNode, onClick?:()=>void, variant?:'primary'|'ghost'}) => (
    <button onClick={onClick} className={
      variant === 'primary'
        ? 'px-4 py-2 rounded-xl bg-white text-black font-medium hover:bg-zinc-200 active:scale-[.99]'
        : 'px-4 py-2 rounded-xl border border-zinc-700 text-zinc-200 hover:bg-zinc-900'
    }>
      {children}
    </button>
  )
    ...
    <Frame title="Safeguard Wills, Deeds & Insurance" subtitle="Community-rooted digital vault + public proof (demo)">
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <p className="text-zinc-300">This demo shows the ForeverDocs flow without collecting sensitive data. We generate a human-readable verification code and a SHA-256 fingerprint of the file, then render a public-safe crest page families can print or bookmark.</p>
        ...
        <div className="rounded-2xl border border-zinc-800 p-4">
          <div className="aspect-video rounded-xl bg-zinc-950 border border-zinc-800 flex items-center justify-center text-zinc-500">Crest Preview Area</div>
          <div className="mt-3 text-sm text-zinc-400">You will see the crest with your family name and verification code after upload.</div>
        </div>
      </div>
    </Frame>
  )
  ...
  if (step === 'upload') return (
    <Frame title="Upload Document (Demo)" subtitle="File is not persisted; URI and digest are simulated for the walkthrough.">
      <div className="grid md:grid-cols-2 gap-6">
        <div>
          <Field label="Family Name (public-safe)"><Input value={familyName} onChange={(e:any)=>setFamilyName(e.target.value)} /></Field>
          <Field label="Contact (email or phone - off-chain, not displayed)"><Input value={ownerContact} onChange={(e:any)=>setOwnerContact(e.target.value)} placeholder="optional" /></Field>
          <div className="grid grid-cols-2 gap-4">
            <Field label="Document Type">
              <Select value={documentType} onChange={(e:any)=>setDocumentType(e.target.value)}>
                {docTypes.map(t => <option key={t} value={t}>{t}</option>)}
              </Select>
            </Field>
            <Field label="Jurisdiction (State/County)"><Input value={jurisdiction} onChange={(e:any)=>setJurisdiction(e.target.value)} /></Field>
          </div>
          <Field label="Choose File (simulated)">
            <div className="flex items-center gap-3">
              <Input type="text" placeholder="e.g., johnson-deed.pdf" value={fileName} onChange={(e:any)=>setFileName(e.target.value)} />
              <Button onClick={()=>demoUpload()}>Use Sample</Button>
            </div>
            <div className="mt-2 text-xs text-zinc-500">In a real build, this would be a file picker writing to Firebase/Drive.</div>
          </Field>
        ...
        <div className="space-y-4">
          ...
          <Outline>
            ...
            <p className="text-sm text-zinc-400 mt-2">Private data (names inside documents, SSNs, DOBs). Only the digest would be anchored on a chain later for timestamped proof.</p>
          </Outline>
        </div>
      </div>
    </Frame>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Outline>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field label="Verification Code"><Input value={verificationCode} readOnly /></Field>
            <Field label="Uploaded At"><Input value={uploadedAt} readOnly /></Field>
            <Field label="File Digest (SHA-256)"><Input value={fileDigest} readOnly /></Field>
            <Field label="Storage URI (off-chain)"><Input value={storageURI} readOnly /></Field>
            <Field label="Family Name"><Input value={familyName} readOnly /></Field>
            <Field label="Document Type"><Input value={documentType} readOnly /></Field>
            <Field label="Jurisdiction"><Input value={jurisdiction} readOnly /></Field>
          </div>
        ...
        <Outline>
          ...
          <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="col-span-2">
              ...
              <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-3 text-zinc-300">{publicNotes}</div>
            </div>
          </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-4">
            <div className="flex items-center justify-center aspect-square rounded-xl border border-zinc-800 bg-zinc-950">
              <div className="text-xs text-center text-zinc-500 px-4">
                QR placeholder
                <div className="mt-2 font-mono text-[10px] break-all">{'{' + '"v":1,"vc":"' + verificationCode + '","t":"' + documentType + '","j":"' + jurisdiction + '"}'}</div>
              </div>
            </div>
        ...
        <Outline>
          <div className="text-sm grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="md:col-span-2">
              ...
              <div className="text-zinc-300">Not anchored in this demo. In production, we would show chain, tx hash, and block number.</div>
            </div>
          </div>
          <div className="flex gap-3 mt-6">
